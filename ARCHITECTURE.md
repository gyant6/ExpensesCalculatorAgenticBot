...
- [ ] Verify with `terraform validate` and `terraform fmt -check`. Terraform is not installed on the dev machine, so the configuration has been reviewed by hand but never parsed
